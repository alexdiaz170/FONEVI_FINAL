...
async function run() {
  ...
  try {
    const [usuarios, socios, periodos, aportes, aporteDetalles, creditos, pagoCuotas, notificaciones, configuraciones, movimientos, solidaridadMovimientos, auditorias, waLogs, acuerdosPago, dividendos, dividendosSocios] = await Promise.all([
    ...
    const backup = {
      ...
      data: {
        usuarios, socios, periodos, aportes, aporteDetalles, creditos, pagoCuotas,
        notificaciones, configuraciones, movimientos, solidaridadMovimientos,
        auditorias, waLogs, acuerdosPago, dividendos, dividendosSocios,
      },
    };
    ...
    console.log(`✓ Backup creado: ${filepath} (${(backup.metadata.totalRegistros.socios + backup.metadata.totalRegistros.usuarios)} registros)`);
  } finally {
